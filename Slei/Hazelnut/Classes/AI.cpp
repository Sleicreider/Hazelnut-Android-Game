...
void AI::DropObjectMovement()
{
	...
	for(int i = 0; i < vecDropObjects.size(); i++)
	{
		...
		else
		{
            ...
            vecDropObjects.at(i).SetActive(false);
            vecDropObjects.at(i).SetPositionX(DataHandler::COLLECT_GAME_SQUIRREL_POSY_START);
            vecDropObjects.at(i).SetPositionY(aiPosX);
            vecInactiveObjects.push_back(std::move(vecDropObjects.at(i)));
...
void AI::SpawnObject()
{
    if(vecInactiveObjects.size() > 0)
    {
        ...
        DropObjectTypeAssignment(vecInactiveObjects[0]);
